...
const HELP_ITEMS = [
  {
    id: 'help-pay-link',
    title: 'Платежная ссылка  - Если клиенты далеко',
  ...
  {
    id: 'help-rent',
    title: 'Заказы с залогом - Если сдаёте в аренду',
          ...
          {isOpen && (
            <>
              ...
              <Dialog.Content asChild key="content">
                ...
                >
                 {/* Header */}
                  {/* Убрали justify-center, чтобы элементы строились слева направо */}
                  <div className="relative flex items-center px-6 py-5 sticky top-0 bg-white rounded-t-[32px] z-10 shrink-0">
                    
                    {/* Убрали text-center, добавили text-left и разрешили тексту занять нужное место */}
                    <Dialog.Title className="text-[18px] font-semibold leading-[24px] tracking-[-0.02em] text-left text-black">
                    ...
                    <Dialog.Close asChild>
                      <button className="absolute right-6 p-2 text-[#838383] hover:text-[#1A1A1A] transition-colors cursor-pointer focus:outline-none">
                        <X size={24} />
                      </button>
                    </Dialog.Close>
                  </div>
                  ...
                  {/* Scrollable list */}
                  {/* Добавили overflow-x-hidden и уменьшили боковой padding (px-4 вместо p-6), чтобы карточка влезла в экран */}
                  <div className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-6 flex flex-col items-center space-y-4 w-full">
                    {HELP_ITEMS.map((item) => (
                      <div 
                        key={item.id} 
                        // Убрали все фиксированные высоты! Теперь серый блок идеально облегает контент (h-fit)
                        className="flex flex-col items-center w-full max-w-[343px] h-fit bg-[#F9F9F9] rounded-[12px] p-[12px]"
                      >
                        {/* 1. Иконка в кружке */}
                        <div className={cn(
                          "w-[48px] h-[48px] rounded-full flex items-center justify-center shrink-0 shadow-sm text-white bg-gradient-to-br",
                          item.gradient
                        )}>
                          {item.icon}
                        </div>

                        {/* 2. Заголовок (Отступ от иконки строго 12px) */}
                        <h4 className="mt-[12px] w-full text-[16px] font-semibold leading-[22px] tracking-[-0.02em] text-center text-[#1A1A1A]">
                          {item.title}
                        </h4>

                        {/* 3. Описание (Отступ от заголовка строго 8px) */}
                        <p className="mt-[8px] w-full text-[14px] font-normal leading-[20px] tracking-[-0.02em] text-center text-[#838383]">
                          {item.description}
                        </p>

                        {/* 4. Белая рамка с фиолетовым текстом (Отступ от описания строго 12px) */}
                        <div className="mt-[12px] w-full bg-white rounded-[8px] py-[12px] px-[16px] border border-black/[0.03]">
                          <p className="w-full text-[12px] font-normal leading-[16px] tracking-[-0.02em] text-center text-[#573DEB]">
                            {item.target}
                          </p>
                        </div>
